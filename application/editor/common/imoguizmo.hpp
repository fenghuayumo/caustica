// Viewport orientation gizmo (adapted for caustica editor).
// Based on ImOGuizmo (https://github.com/fknfilewalker/imoguizmo).
#pragma once
#include <imgui.h>
#include <cmath>
#include <cstring>
...
namespace ImOGuizmo {
	...
	namespace internal {
		...
		{
			switch (coordinateSystem) {
			case CoordinateSystem::YZX: return { 1, 2, 0 };
			case CoordinateSystem::ZXY: return { 2, 0, 1 };
			case CoordinateSystem::XZY: return { 0, 2, 1 };
			case CoordinateSystem::YXZ: return { 1, 0, 2 };
			case CoordinateSystem::ZYX: return { 2, 1, 0 };
			default: return { 0, 1, 2 }; // CoordinateSystem::XYZ
			}
		}

		inline ImVec3 axisVector(const int axis, const float sign = 1.0f)
		{
			if (axis == 0) return { sign, 0.0f, 0.0f };
			if (axis == 1) return { 0.0f, sign, 0.0f };
		...
		inline void lookAt(ImVec3 const& eye, ImVec3 const& at, ImVec3 const& up, float* viewMatrix, const CoordinateSystem coordinateSystem)
		{
			...
			viewMatrix[0] = r[0]; viewMatrix[1] = u[0]; viewMatrix[2] = zSign * f[0]; viewMatrix[3] = 0.0f;
			viewMatrix[4] = r[1]; viewMatrix[5] = u[1]; viewMatrix[6] = zSign * f[1]; viewMatrix[7] = 0.0f;
			viewMatrix[8] = r[2]; viewMatrix[9] = u[2]; viewMatrix[10] = zSign * f[2]; viewMatrix[11] = 0.0f;
			viewMatrix[12] = -dot(r, eye); viewMatrix[13] = -dot(u, eye); viewMatrix[14] = -zSign * dot(f, eye); viewMatrix[15] = 1.0f;
		}
		...
		inline void lookAtSelection(const ImVec3& pivotPos, const float pivotDistance, const int selection, float* viewMatrix, const CoordinateSystem coordinateSystem)
		{
		...
		inline bool orbitView(float* const viewMatrix, const ImVec2 mouseDelta, const float pivotDistance, const CoordinateSystem coordinateSystem)
		{
			if (pivotDistance <= 0.0f || (mouseDelta.x == 0.0f && mouseDelta.y == 0.0f)) return false;
	...
	{
		const ImGuiWindowFlags flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoBringToFrontOnFocus | ((background != true) ? ImGuiWindowFlags_NoBackground : ImGuiWindowFlags_None);
	...
	inline bool DrawGizmo(float* const viewMatrix, const float* const projectionMatrix, const float pivotDistance = 0.0f, const CoordinateSystem coordinateSystem = CoordinateSystem::XYZ) {
		...
		SetDrawList(internal::config.mDrawList);
		if (config.hoverCircleColor != 0 && (hovered || dragging)) internal::config.mDrawList->AddCircleFilled(center, hoverCircleRadius, config.hoverCircleColor);
		...
		std::array<std::pair<int, float>, 6> pairs = { { {0, xAxis.w}, {1, yAxis.w}, {2, zAxis.w}, {3, -xAxis.w}, {4, -yAxis.w}, {5, -zAxis.w} } };
		...
		for (const auto& [fst, snd] : pairs) {
			switch (fst) {
			case 0: // +x axis
				internal::drawPositiveLine(center, ImVec2{ xAxis.x, -xAxis.y }, xPositiveCloser ? config.xCircleFrontColor : config.xCircleBackColor, positiveRadius, lineThickness, "X", !dragging && selection == 0);
				continue;
			case 1: // +y axis
				internal::drawPositiveLine(center, ImVec2{ yAxis.x, -yAxis.y }, yPositiveCloser ? config.yCircleFrontColor : config.yCircleBackColor, positiveRadius, lineThickness, "Y", !dragging && selection == 1);
				continue;
			case 2: // +z axis
				internal::drawPositiveLine(center, ImVec2{ zAxis.x, -zAxis.y }, zPositiveCloser ? config.zCircleFrontColor : config.zCircleBackColor, positiveRadius, lineThickness, "Z", !dragging && selection == 2);
				continue;
			case 3: // -x axis
				internal::drawNegativeLine(center, ImVec2{ xAxis.x, -xAxis.y }, !xPositiveCloser ? config.xCircleFrontColor : config.xCircleBackColor, negativeRadius, !dragging && selection == 3);
				continue;
			case 4: // -y axis
				internal::drawNegativeLine(center, ImVec2{ yAxis.x, -yAxis.y }, !yPositiveCloser ? config.yCircleFrontColor : config.yCircleBackColor, negativeRadius, !dragging && selection == 4);
				continue;
			case 5: // -z axis
				internal::drawNegativeLine(center, ImVec2{ zAxis.x, -zAxis.y }, !zPositiveCloser ? config.zCircleFrontColor : config.zCircleBackColor, negativeRadius, !dragging && selection == 5);
				continue;
			default: break;
			}
		}
		...
		if (config.drag) {
			if (internal::interaction.dragging && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
				// Avoid ImVec2 operators: caustica::math ADL overloads can steal them.
				const ImVec2 click = internal::interaction.clickMousePos;
				const ImVec2 d(mousePos.x - click.x, mousePos.y - click.y);
				...
				if (d.x * d.x + d.y * d.y > dragThresholdSq) {
					...
					const ImVec2 orbitDelta(md.x * config.dragSensitivity, md.y * config.dragSensitivity);
					return internal::orbitView(viewMatrix, orbitDelta, pivotDistance, coordinateSystem);
				}
			}
