...
void GaussianSplatPass::CreatePipeline(const RenderTargets& renderTargets)
{
    ...
    m_rasterVertexShader = m_shaderFactory->CreateShader("app/engine/ProcessingPasses/GaussianSplatRaster.hlsl", "vs_main", &rasterShadowMacros, nvrhi::ShaderType::Vertex);
    m_rasterPixelShader = m_shaderFactory->CreateShader("app/engine/ProcessingPasses/GaussianSplatRaster.hlsl", "ps_main", &rasterShadowMacros, nvrhi::ShaderType::Pixel);

    std::vector<donut::engine::ShaderMacro> hybridShadowMacros = {
        donut::engine::ShaderMacro({ "GAUSSIAN_SPLAT_HYBRID_SHADOWS", "1" })
    };
    m_hybridVertexShader = m_shaderFactory->CreateShader("app/engine/ProcessingPasses/GaussianSplatRaster.hlsl", "vs_main", &hybridShadowMacros, nvrhi::ShaderType::Vertex);
    m_hybridPixelShader = m_shaderFactory->CreateShader("app/engine/ProcessingPasses/GaussianSplatRaster.hlsl", "ps_main", &hybridShadowMacros, nvrhi::ShaderType::Pixel);

    std::vector<donut::engine::ShaderMacro> sortKeyMacros = {
        donut::engine::ShaderMacro({ "GAUSSIAN_SPLAT_SORT_KEYS", "1" })
    };
    m_sortKeyShader = m_shaderFactory->CreateShader("app/engine/ProcessingPasses/GaussianSplatRaster.hlsl", "cs_sort_keys", &sortKeyMacros, nvrhi::ShaderType::Compute);
