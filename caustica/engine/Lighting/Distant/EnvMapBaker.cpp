...
void EnvMapBaker::CreateRenderPasses(std::shared_ptr<ShaderDebug> shaderDebug, std::shared_ptr<donut::engine::ShaderFactory> shaderFactory, std::shared_ptr<ComputePipelineBaker> computePipelineBaker)
{
    ...
    m_lowResPrePassLayerCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/EnvMapBaker.hlsl", "LowResPrePassLayerCS", &shaderMacros, nvrhi::ShaderType::Compute);
    m_baseLayerCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/EnvMapBaker.hlsl", "BaseLayerCS", &shaderMacros, nvrhi::ShaderType::Compute);
    m_MIPReduceCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/EnvMapBaker.hlsl", "MIPReduceCS", &shaderMacros, nvrhi::ShaderType::Compute);
    ...
    if (m_BC6UCompressionEnabled)
    {
        ...
        m_BC6UCompressLowCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/BC6UCompress.hlsl", "CSMain", &smQ0, nvrhi::ShaderType::Compute);
        m_BC6UCompressHighCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/BC6UCompress.hlsl", "CSMain", &smQ1, nvrhi::ShaderType::Compute);
    ...
    {
        m_brdfLUTCS = shaderFactory->CreateShader("app/engine/Lighting/Distant/BRDFLUTGenerator.hlsl", "main", nullptr, nvrhi::ShaderType::Compute);
    ...
    if (m_enableRasterPrecompute)
    {
        ...
        if (m_computePipelineBaker)
        {
            m_ggxPrefilterVariant = m_computePipelineBaker->CreateVariant(
                "../engine/Lighting/Distant/CubemapProcessing.hlsl",  // source path relative to ShadersPath (caustica/Shaders)
