...
void main( uint2 dispatchThreadID : SV_DispatchThreadID )
{
...
#if 0 // all of this moved to primary BUILD pass of Path Tracer
    ...
    float3 rayOrigin; float3 rayDir; uint vertexIndex; float sceneLength; float3 pathThp; float3 motionVectors;
    StablePlanesContext::UnpackStablePlane( sp, vertexIndex, rayOrigin, rayDir, sceneLength, pathThp, motionVectors );

    // if ( any( u_MotionVectors[pixelPos] != float4(motionVectors, 0) ) )
    //     DebugPixel( pixelPos.xy, float4( 1, 0.2, 0, 1 ) );
    //
    ...
    bool hitSurface = isfinite(rayOrigin.x); //hit.isValid();// && hit.getType() == HitType::Triangle;
...
/*    if (g_Const.ptConsts.useReSTIRDI || g_Const.ptConsts.useReSTIRGI)
    ...
    if (g_Const.debug.debugViewType==(int)DebugViewType::VBufferMotionVectors)
        DebugPixel( pixelPos, float4( 0.5.xxx + u_MotionVectors[pixelPos].xyz * float3(0.2, 0.2, 10), 1) );
    else if (g_Const.debug.debugViewType==(int)DebugViewType::VBufferDepth)
        DebugPixel( pixelPos, float4( u_Depth[pixelPos].xxx * 100.0, 1 ) );
