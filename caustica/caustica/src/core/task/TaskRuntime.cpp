...
void Pipe::scheduleNextUnlocked()
{
    ...
    {
        std::lock_guard<std::mutex> qlock(rt.queueMutex);
        rt.queues[prio].push_back(std::move(next));
    }
    rt.wakeCv.notify_one();
}
...
void initialize(uint32_t numWorkers, uint32_t reservedThreads)
{
    ...
    rt.workerCount = numWorkers;
    rt.running.store(true, std::memory_order_release);
    rt.workers.clear();
    rt.workers.reserve(numWorkers);
    for (uint32_t i = 0; i < numWorkers; ++i)
        rt.workers.emplace_back(detail::workerMain);

    caustica::info("TaskRuntime initialized with %u worker threads", numWorkers);
}
