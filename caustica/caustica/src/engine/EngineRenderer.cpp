...
void EngineRenderer::createPathTracer(const PathTracerSessionParams& session)
{
    ...
    m_pathTracingContext = std::make_unique<render::PathTracingContext>(render::PathTracingContext{
        ...
        .envMapProcessor = lighting.envMapProcessor(),
        .lightSamplingCache = lighting.lightSamplingCache(),
        .materialGpuCache = lighting.materialGpuCache(),
        .opacityMicromapBuilder = lighting.opacityMicromapBuilder(),
        .computePipelineRegistry = lighting.computePipelineRegistry(),
