...
void caustica::render::WorldRenderer::createRenderPasses( bool& exposureResetRequired, nvrhi::CommandListHandle initializeCommandList )
{
    ...
    createRtxdiRenderPasses();
    createAccumulationRenderPasses();
    createGaussianTemporalRenderPasses();
    createPostProcessRenderPasses();

    if (!createPTPipeline())
        { assert(false); }

    createLightingRenderPasses(initializeCommandList);
    prepareGaussianSplatPasses();
    createDenoiserRenderPasses();
}
