...
#include "render/rtxdi/internal/RtxdiParameters.h"
#include "render/rtxdi/internal/LightSampling/RISBufferSegmentAllocator.h"

#ifdef _MSC_VER
#pragma warning( disable : 5045) // QSpectre notification warnings in /Wall
#endif
...
namespace caustica::rtxdi_internal
{
    ...
    void ReGIRContext::InitializeOnion(const ReGIRStaticParameters& params)
    {
        ...
        m_regirOnionCalculatedParameters.regirOnionCells = uint32_t(totalCells);
    ...
    void ReGIRContext::ComputeOnionJitterCurve()
    {
        ...
        for (const auto& layerGroup : m_regirOnionCalculatedParameters.regirOnionLayers)
        {
            for (int layerIndex = 0; layerIndex < layerGroup.layerCount; layerIndex++)
            {
                ...
                {
                    const auto& ring = m_regirOnionCalculatedParameters.regirOnionRings[uint64_t(layerGroup.ringOffset + ringIndex)];
    ...
    ReGIRGridCalculatedParameters caustica::rtxdi_internal::ReGIRContext::GetReGIRGridCalculatedParameters() const
    {
        return m_regirGridCalculatedParameters;
    }

    ReGIROnionCalculatedParameters caustica::rtxdi_internal::ReGIRContext::GetReGIROnionCalculatedParameters() const
    {
        return m_regirOnionCalculatedParameters;
    }

    uint32_t caustica::rtxdi_internal::ReGIRContext::GetReGIRCellOffset() const
    {
        return m_regirCellOffset;
    }

    uint32_t caustica::rtxdi_internal::ReGIRContext::GetReGIRLightSlotCount() const
    {
        switch (m_regirStaticParameters.Mode)
        {
        case ReGIRMode::Grid:
