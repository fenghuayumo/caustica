	...
	uint3 groupThreadID : SV_GroupThreadID)
{
    ...
    OutputTexture.GetDimensions(TextureSizeInBlocks.x, TextureSizeInBlocks.y, TextureSizeInBlocks.z);
    float2 TextureSizeRcp = 1.0 / float2(TextureSizeInBlocks.xy*4.0);

	if (all(blockCoord < TextureSizeInBlocks.xy))
	{
		...
		float4 block0X = SrcTexture.GatherRed(PointSampler, float3(block0UV, faceIndex));
		float4 block1X = SrcTexture.GatherRed(PointSampler, float3(block1UV, faceIndex));
		float4 block2X = SrcTexture.GatherRed(PointSampler, float3(block2UV, faceIndex));
		float4 block3X = SrcTexture.GatherRed(PointSampler, float3(block3UV, faceIndex));
		float4 block0Y = SrcTexture.GatherGreen(PointSampler, float3(block0UV, faceIndex));
		float4 block1Y = SrcTexture.GatherGreen(PointSampler, float3(block1UV, faceIndex));
		float4 block2Y = SrcTexture.GatherGreen(PointSampler, float3(block2UV, faceIndex));
		float4 block3Y = SrcTexture.GatherGreen(PointSampler, float3(block3UV, faceIndex));
		float4 block0Z = SrcTexture.GatherBlue(PointSampler, float3(block0UV, faceIndex));
		float4 block1Z = SrcTexture.GatherBlue(PointSampler, float3(block1UV, faceIndex));
		float4 block2Z = SrcTexture.GatherBlue(PointSampler, float3(block2UV, faceIndex));
		float4 block3Z = SrcTexture.GatherBlue(PointSampler, float3(block3UV, faceIndex));
